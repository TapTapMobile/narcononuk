var VideoController = function(opts) {
    if (opts)
        for (var key in opts) this[key] = opts[key];
}
VideoController.prototype = (function() {
    var me = {};
    var autogeneratedId = 0;
    me.init = function(aDomElem, aOpts) {
        if (globalGetParams && globalGetParams.headless) return {
            on: function() {},
            one: function() {},
            trigger: function() {},
            el: function() {},
        ...
        var $elem = (aDomElem instanceof jQuery) ? aDomElem : $(aDomElem),
            vid = (aDomElem instanceof jQuery) ? aDomElem[0] : $elem[0];
        var domElemId = vid.id;
        if (!domElemId) {
            domElemId = 'gsc_video_controller_' + autogeneratedId++;
            vid.id = domElemId;
        }
        logger.debug('videoJS DOM elem ID: ' + domElemId);
        if ($('html').hasClass('device-android')) {
            $elem.attr('controls', 'controls');
            vid.on = vid.addEventListener;
            vid.one = function(aEvent, aFunc) {
                var theFunc = function() {
                    aFunc.apply(this, arguments);
                    vid.removeEventListener(aEvent, theFunc);
                };
                vid.addEventListener(aEvent, theFunc);
            }
            vid.trigger = function() {
                $elem.trigger.apply($elem, arguments);
            }
            vid.el = function() {
                return vid;
            }
            ...
            if (navigator.userAgent.toLowerCase().indexOf('firefox') > -1 && navigator.platform.indexOf("Win") > -1 && $elem.find('source').eq(0).attr('type') != 'video/webm') {
                opts.techOrder = ["flash", "html5"];
            }
            vid = videojs(domElemId, opts);
            if (opts.showPosterOnVideoEnnd) {
...
$(document).ready(function() {
    videojs.options.strings = {
        'en_GB': {
            'Play': 'Play',
            'Pause': 'Pause',
            'Mute': 'gcui_scientology:st-Mute',
            'Unmute': 'gcui_scientology:st-Mute',
            'Current Time': 'gcui_scientology:st-Time',
            'Duration Time': 'gcui_scientology:st-Duration',
            'Fullscreen': 'gcui_scientology:st-Fullscreen',
            'Non-Fullscreen': 'gcui_scientology:st-non-Fullscreen'
        }
    };
    videojs.options.language = 'en_GB';
    videojs.options.flash.swf = 'theme/swf/video-js.swf';
    if (!$('html').hasClass('device-ipad')) {
        $('[data-gsc-setup]').each(function() {});
    }
});
globalVideoController = new VideoController();;
new function() {
    ...
    function reportInternalError() {
        reportErrors = false;
        try {
            _tr.push(['_trackEvent', 'Errors', 'scn-after-event-feedback-form', arguments[0] + '|' + arguments[1] + '|' + arguments[2] + '|' + arguments[3] + '|' + arguments[4]]);
        } catch (throw_away) {}
    }
    ...
    function reportError(errorMsg, url, lineNumber, colNumber, errorObj) {
        ...
        try {
            _tr.push(['_trackEvent', 'Errors', 'scn-after-event-feedback-form', '"' + errorMsg + '" @ ' + url + ':' + lineNumber +
                (colNumber !== void 0 ? ', ' + colNumber : '') +
                (errorObj !== void 0 && window.JSON && typeof JSON.stringify === 'function' ? ', ' + JSON.stringify(errorObj) : '')
            ]);
        } catch (e) {
            try {
        ...
        $(document).ready(function() {
            if (typeof $(document).ajaxError === 'function') {
                $(document).ajaxError(function(e, req, settings, thrownError) {
                    ...
                    recursionDetector = 1;
                    reportError('jQuery ajax error: ' + e + (thrownError ? ', ' + thrownError : ''), settings && settings.url, 0, 0, settings);
                    recursionDetector = 0;
                });
            }
        });
...
window.Modernizr = function(a, b, c) {
    ...
    for (var I in s) A(s, I) && (x = I.toLowerCase(), e[x] = s[I](), v.push((e[x] ? "" : "no-") + x));
    return e.addTest = function(a, b) {
        if (typeof a == "object")
            ...
            a = a.toLowerCase();
            if (e[a] !== c) return e;
            b = typeof b == "function" ? b() : b, typeof f != "undefined" && f && (g.className += " modernizr-" + (b ? "" : "no-") + a), e[a] = b
        }
        return e
    }, B(""), i = k = null, e._version = d, e._prefixes = n, e._domPrefixes = q, e._cssomPrefixes = p, e.testProp = function(a) {
        return F([a])
    }, e.testAllProps = H, e.testStyles = y, e.prefixed = function(a, b, c) {
        return b ? H(a, b, c) : H(a, "pfx")
    }, g.className = g.className.replace(/(^|\s)no-js(\s|$)/, "$1$2") + (f ? " modernizr-js modernizr-" + v.join(" modernizr-") : ""), e
}(this, this.document),
function(a, b) {
    function l(a, b) {
...
if (!window.$) {
    var documentReadyHolder = [],
        preDocReadyBindOrExecHolder = [],
        preDocReadyTriggerHolder = [];
    var $ = (function() {
        return function(aParam) {
            ...
            if (aParam != document) {
                ...
                return {
                    ready: function(aFunc) {
                        documentReadyHolder.push(aFunc)
                    },
                    bindOrExecute: function() {
                        preDocReadyBindOrExecHolder.push([aParam, arguments])
                    },
                    trigger: function() {
                        preDocReadyTriggerHolder.push([aParam, arguments])
                    },
                };
            }
        }
...
new function() {
    ...
    if (ua.toLowerCase().indexOf('safari') != -1) {
        if (ua.toLowerCase().indexOf('chrome') > -1) {} else {
            globalIsSafari = true;
            document.documentElement.className += ' browser-safari';
        }
    }
    globalIsIE = (document.documentElement.className.indexOf('ie678') > -1);
    var mobile_user_agent_strings = ['Android', 'iPhone', 'iPad', 'iPod', 'Opera Mobi', 'BlackBerry', 'AvantGo', 'DoCoMo', 'KDDI', 'Vodafone', 'J-PHONE', 'DDIPOCKET', 'PDXGW', 'ASTEL', 'EudoraWeb', 'Windows CE', 'Minimo', 'OPWV-SDK', 'UP.Browser', 'PLink', 'Plucker', 'NetFront', 'WM5 PIE', 'Xiino', 'BenQ', 'Cricket', 'IEMobile', 'Nintendo Wii', 'nook browser', 'Nook', 'Kindle'];
...
if (!globalGetParams.geoDebugIP) {
    globalGeolocationData = cookieMonster.get('geolocatedNN');
...
if (!globalGeolocationData) {
    $(document).ready(function() {
        ...
        $.ajax({
            url: 'http://' + globaldata_domain + '/gcui-globaldata/geolocate-api.action',
            dataType: 'jsonp',
            cache: true,
            data: ajaxData,
            success: function(data) {
                ...
                cookieMonster.set('geolocatedNN', JSON.stringify(globalGeolocationData), void 0, '/');
                $(document).trigger('geolocated');
                $(document).trigger('org-selected', globalGeolocationData.items && globalGeolocationData.items.id);
                $(document).trigger('gcui_org_preference_set', {
                    result: globalGeolocationData
                });
            },
            error: function() {
...
function smoothScrollTo(aToXPosition, aCallback) {
    ...
    globalAnimationController.animate($('body'), {
        'margin-top': 0
    }, '.6s cubic-bezier(0.165, 0.840, 0.440, 1.000)', function() {
...
function html5GeolocateIfPossibleCookieResults() {
    globalHtml5GeolocationData = cookieMonster.get('html5_geolocatedNN');
    ...
    if (Modernizr.geolocation && !globalGetParams.geoDebugIP) navigator.geolocation.getCurrentPosition(function(aPos) {
        logger.debug('HTML5 geolocation successful');
        logger.debug(aPos);
        globalHtml5GeolocationData = {
            estimated_location: aPos.coords
        }
        $.ajax({
            ...
            success: function(data) {
                ...
                delete globalGeolocationData.items.geo_accuracy;
                delete globalGeolocationData.items.incomm_id;
                cookieMonster.set('html5_geolocatedNN', JSON.stringify(globalHtml5GeolocationData), void 0, '/');
                $(document).trigger('html5_geolocated');
            },
            error: function() {
...
function responsiveSliderDetermineFirstSlide() {
    ...
    if (globalGetParams.video || globalGetParams['video-play']) {
        ...
        var slideMatching = Array.prototype.reduce.call(document.querySelectorAll('#header .slideshow-main slide .video-js'), function(m, v) {
            if (m != null) return m;
...
function slideshowResize(aHeight, hasLeftSidebar) {
    ...
    if (document.querySelector) {
        ...
        if (hasLeftSidebar && (!location.hash.match(/#slide/) || location.hash.match(/#slide1$/)) && !globalGetParams.video) {
            myWrapper.style.opacity = 1;
...
function wireupTotalTouchControl(aClass, aCallback, aTransparent) {
    ...
    for (var elem in {
        ...
        document.addEventListener(elem, function(e) {
            if (e && e.target && e.target.classList && (e.target.classList.contains(aClass) || (e.target.parentNode && e.target.parentNode.classList && e.target.parentNode.classList.contains(aClass)))) {
                if (!aTransparent) {
                    e.stopPropagation();
        ...
        Object.defineProperty(c, "activeCues", {
            get: function() {
                ...
                if (d.length !== this.Ga.length) A = f;
                    ...
                    for (a = 0; a < d.length; a++) - 1 === ua.call(this.Ga, d[a]) && (A = f);
                this.Ga = d;
                r.qb(this.Ga);
                return r
            },
...
(function(vjs) {
    (function() {
        var hasLocalStorage;
        try {
            window.localStorage.setItem('modernizr', 'modernizr');
            window.localStorage.removeItem('modernizr');
        ...
        try {
            var val;
            if (hasLocalStorage) {
                val = window.localStorage.getItem('vjs');
            } else {
                val = document.cookie.replace(/(?:^|.*;)\s*vjs\s*=\s*([^;]*).*|^.*$/, '$1') || '';
        ...
        function saveOptions() {
            if (hasLocalStorage) {
                window.localStorage.setItem('vjs', JSON.stringify(options));
            } else {
                document.cookie = 'vjs=' + encodeURIComponent(JSON.stringify(options)) + '; path=/';
            }
        }
        vjs.plugin('sticky', function() {
            var vid = this;
            vid.ready(function() {
                (options.volume === undefined) || vid.volume(options.volume);
                vid.on('volumechange', function() {
                    options.volume = vid.volume();
                    saveOptions();
                });
                var st = vid.textTracks(),
                    tt;
                for (var i = 0, l = st.length; i < l; i++) {
                    tt = st[i];
                    if (options.subtitles !== undefined) {
                        if (options.subtitles === 2) {
                            tt.show();
                        } else if (options.subtitles === 1) {
                            tt.hide();
                        } else {
                            tt.disable();
                        }
                ...
                if (tt) {
                    var stb = vid.controlBar.getChild('subtitlesButton');
                    stb.on('click', function(e) {
                        ...
                        options.subtitles = tt.mode();
                        saveOptions();
                    });
                    stb = stb.children()[0];
                    if (stb) {
                        stb = stb.children();
                        for (var i = 0, l = stb.length; i < l; i++) {
                            stb[i].on('click', function(e) {
                                options.subtitles = tt.mode();
                                saveOptions();
                            });
                        }
    ...
    (function() {
        ...
        function onEndedOnce(e) {
            _ga_send('Videos', 'Video Completed', getVideoId(this));
        }

        function onPlayOnce(e) {
            _ga_send('Videos', 'Play', getVideoId(this));
        }

        function onProgressOnce(e) {
            var progress;
            try {
            ...
            if (progress >= 0.2) {
                this.off('timeupdate', onProgressOnce);
                this.on('timeupdate', onProgressTwice);
                _ga_send('Videos', 'Watched 20%', getVideoId(this));
            }
        }

        function onProgressTwice(e) {
            var progress;
            try {
                progress = this.currentTime() / this.duration();
            } catch (e) {
            ...
            if (progress >= 0.8) {
                this.off('timeupdate', onProgressTwice);
                _ga_send('Videos', 'Watched 80%', getVideoId(this));
            }
        }

        function setTracking() {
            this.off('ended', onEndedOnce);
            this.off('play', onPlayOnce);
            this.off('timeupdate', onProgressOnce);
            this.off('timeupdate', onProgressTwice);
            this.one('ended', onEndedOnce);
            this.one('play', onPlayOnce);
            this.on('timeupdate', onProgressOnce);
        }
        vjs.plugin('tracking', function() {
            this.ready(setTracking);
            this.on('src', setTracking);
        });
    })();
    var vjsOptions = vjs['options'];
    vjsOptions['nativeControlsForTouch'] = true;
    if (navigator.userAgent.toLowerCase().indexOf('firefox') > -1 && navigator.platform.indexOf('Win') > -1) {
        vjsOptions['techOrder'] = ['flash', 'html5'];
    }
    var vjsPlayer = vjs['Player'];

    function mjsPlayer() {
        vjsPlayer.apply(this, arguments);
        this.sticky();
        this.tracking();
    }

    function F() {}
    F.prototype = vjsPlayer.prototype;
    mjsPlayer.prototype = new F();
    for (var key in vjsPlayer) {
        if (vjsPlayer.hasOwnProperty(key)) {
            mjsPlayer[key] = vjsPlayer[key];
        }
    }
    mjsPlayer.vjsPlayer = vjsPlayer;
    vjs['Player'] = mjsPlayer;
    mjsPlayer.prototype.src = function() {
        var ret = vjsPlayer.prototype.src.apply(this, arguments);
        if (ret === this) {
            this.trigger('src');
        }
        return ret;
...
$(document).ready(function() {
    $(window).on('resize', function() {
        $('.js-gcui-responsive-width').width($('#content_container').width());
    });
    $('.js-gcui-responsive-width').width($('#content_container').width());
    $('a.clickable-pseudo-elem').on('click', function(e) {
        location = $(this).attr('href');
    });
    if (!globalIsIOS) {
        $(document).on('click', '.pass-the-click', function(e) {
            location = $(this).find('a').attr('href');
        });
    } else {
        $(document).on('click', '.pass-the-click', function(e) {
            gcui_send_click($(this).find('a')[0]);
        });
    }
});
if (!window.logger) {
...
function gcui_log(something) {
    logger.debug(something);
}

function attachCssAnimationCallback($aDomElem, aFunc) {
    $aDomElem.on('webkitTransitionEnd oTransitionEnd otransitionend transitionend msTransitionEnd MSTransitionEnd', function(e) {
        if (e.target != $aDomElem[0]) return true;
        $aDomElem.unbind('webkitTransitionEnd oTransitionEnd otransitionend transitionend msTransitionEnd MSTransitionEnd');
        aFunc();
    });
}
    ...
    $.fn.equalHeights = function(px) {
        $(this).each(function() {
            ...
            $(this).children().each(function(i) {
                if ($(this).hasClass('clearfix')) return;
                if ($(this).height() > currentTallest) {
                    currentTallest = $(this).height();
                }
            });
            if ($('html').hasClass('ie6')) {
                $(this).children().not('.clearfix').css({
...
new function() {
    $(document).ready(function() {
        ...
        if ($.browser.webkit) {
            ...
            vP = "-ms-";
            vTransform = '-ms-transform';
        } else if ($.browser.mozilla) {
            vP = "-moz-";
            vTransform = '-moz-transform';
            transitionEnd = "transitionend";
        } else if ($.browser.opera) {
            vP = "-o-";
            vTransform = '-o-transform';
        ...
        function animate2_(object, cssProperties, ms, _easing, callback) {
            ...
            if (cssTransitionsSupported && typeof cssProperties.width == 'undefined') {
                ...
                var transatePropsChanged = false;
                for (var prop in props) {
                    if (typeof cssProperties[prop] != 'undefined' && cssProperties[prop].match(/[+-]=/)) {
                        transatePropsChanged = true;
                        var dir = cssProperties[prop].replace(/=.*$/, '');
                        var amount = parseInt(cssProperties[prop].replace(/^.*=/, '').replace(/px/, ''));
                        var existingCoords = {
                            left: 0,
                            top: 0
                        };
                        var currentTransform = object.css(vTransform);
                        if (vTransform == '-moz-transform') currentTransform = object.css('transform');
                        gcui_log(translateParts);
                        if (currentTransform && currentTransform.match(/translate3d/)) {
                            var translateParts = /translate3d\((.*),(.*),(.*)\)/.exec(currentTransform);
                            if (translateParts && translateParts.length > 3) {
                                existingCoords.left = translateParts[1].replace(/px/, '');
                                existingCoords.top = translateParts[2].replace(/px/, '');
                            }
                        } else if (currentTransform && currentTransform.match(/matrix/)) {
                            var matrixParts = currentTransform.split(',');
                            existingCoords.left = matrixParts[4].trim().replace(/px/, '');
                            existingCoords.top = matrixParts[5].trim().replace(/px/, '');
                        }
                        delete cssProperties[prop];
                        transateProps[prop] = existingCoords[prop] ? (dir == '+' ? parseInt(existingCoords[prop]) + amount : parseInt(existingCoords[prop]) - amount) : (dir == '+' ? parseInt(amount) : -parseInt(amount));
                    }
                    if (typeof cssProperties[prop] != 'undefined' && cssProperties[prop] == 0) {
                        transateProps[prop] = 0;
                        transatePropsChanged = true;
                    }
                }
                if (transatePropsChanged) {
                    cssProperties[vTransform] = 'translate3d(' +
                        transateProps.left + 'px,' +
                        transateProps.top + 'px,' + '0px' + ')';
                }
                gcui_log('applying: ' + JSON.stringify(cssProperties));
...
function duplicateBody() {
    ...
    screenshot.querySelector('body').className += ' content-blurred';
    var scripts = screenshot.querySelectorAll('script');
...
if (!window.globalNoCloning) {
    var cloneScroller = document.querySelector('.clone-scroller');
    document.querySelector('.clone-scroller').innerHTML = '<iframe style="width: 100%" id="viewer"></iframe>';
    var iframe = document.getElementById('viewer');
    var duplicate = duplicateBody();
    window.URL = window.URL || window.webkitURL;
    var obj_url = window.URL.createObjectURL(duplicate);
    iframe.setAttribute('src', obj_url);
    iframe.height = document.body.getBoundingClientRect().height + 'px';
    cloneScroller.scrollTop = document.documentElement.scrollTop || document.body.scrollTop;
    ...
    me.animate2d = function(aDomElem, aTransform2d, aTransition, aCallbackFunc) {
        ...
        _.defer(function() {
            if (Modernizr.csstransitions && Modernizr.csstransforms3d) {
                if (aTransition != 'none') {
                    aDomElem.css(Modernizr.prefixed('transition'), transitionTransformProps[Modernizr.prefixed('transform')] + ' ' + aTransition);
                } else {
    ...
    me.animate = function(aDomElem, aProps, aTransition, aCallbackFunc) {
        ...
        _.defer(function() {
            if (Modernizr.csstransitions) {
                if (aTransition != 'none') {
                    aDomElem.css(Modernizr.prefixed('transition'), 'all ' + aTransition);
                } else {
                    aDomElem.css(Modernizr.prefixed('transition'), '');
                }
            }
...
if ($('body').hasClass('narconon-center-site')) {
    ...
    $(document).ready(function() {
        $(document).trigger('custom_geolocation_completed', [globalCurrentCenter]);
    });
} else {
    if ($('body').hasClass('narconon-region-site')) {
        var bestNumber = globalCurrentCenter.Csitollfreetelephonenumber || globalCurrentCenter.Tollfreetelephonenumber || globalCurrentCenter.Phone || globalCurrentCenter.Facsimiletelephonenumber || '';
...
$(document).ready(function() {
    ...
    $(document).on('click', '.onclick-inside', function(e) {
        if (e && e.target && e.target.tagName === 'A') return true;
        if (e.ctrlKey || e.keyCode == 224 || e.keyCode == 17 || e.keyCode == 91 || e.keyCode == 03) {
            window.open($(this).attr('href'));
        } else {
            location = $(this).attr('href');
        }
    });
    $(document).on('click', '.more-opener', function(e) {
        e.preventDefault();
        $(this).addClass('activate');
        $(this).parents('p').siblings('.more-hider').addClass('activate');
        return false;
    });
    if (globalGetParams.crop) {
        document.addEventListener('click', function(e) {
            if (!($(e.target).is('.slide img'))) return true;
            var $this = $(e.target);
            e.preventDefault();
            e.stopPropagation();
            e.stopImmediatePropagation();
            var width = parseInt($this.attr('data-width'));
            window.open('http://f.edit.firechrome.org/tools/cropper/?q=/imagecache/cropfit@w=' +
                Math.floor(width * ($this.parents('#lightbox').length ? 1.5 : 1.5)) + '@qa=85@cr=0,0,' + width + ',' + parseInt($this.attr('data-height')) + '/' + $this.attr('src').replace(/^.*org\//, '').replace(/^files/, 'data/www.narconon.org/files'));
            if (globalGetParams.crop == '1') location.hash = '#"'
            return false;
        }, true);
    }
});